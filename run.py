...
parser.add_argument(
    ...
    default="lsas_senet",
    choices=model_names,
    help="model architecture: " + " | ".join(model_names) + " (default: lsas_senet)",
)
...
parser.add_argument(
    ...
    type=str,
    default="BasicBlock",
    help="the building block for Resnet and Preresnet: BasicBlock, Bottleneck (default: Basicblock for cifar10/cifar100/stl10)",
)
parser.add_argument(
    "--cardinality", type=int, default=8, help="Model cardinality (group)."
)
parser.add_argument(
    "--widen-factor", type=int, default=4, help="Widen factor. 4 -> 64, 8 -> 128, ..."
)
parser.add_argument(
    "--growthRate", type=int, default=12, help="Growth rate for DenseNet."
)
parser.add_argument(
    "--compressionRate",
    type=int,
...
parser.add_argument(
    ...
    default="normal",
    type=str,
    help="info for tasks and identifying checkpoints",
)
...
# Validate dataset
assert (
    args.dataset == "cifar10" or args.dataset == "cifar100" or args.dataset == "stl10"
), "Dataset can only be cifar10, cifar100 or stl10."
...
def main():
    ...
    args.checkpoint = (
        args.checkpoint + "/" + args.dataset + "/" + args.arch + "/" + args.info
    )
    ...
    if args.dataset == "stl10":
        trainset = datasets.STL10(
            root="../datasets", download=True, split="train", transform=transform_train
        )
        ...
        testloader = data.DataLoader(testset, batch_size=256)
    else:
        trainset = dataloader(
    ...
    if args.arch.endswith("lsas_senet") or args.arch.endswith("senet"):

        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if d_p.dim() == 4 and sz[1] != 1:  # we do not consider dw conv
                    ...
                    sz = p.data.size()
                    w = p.data.reshape(sz[0], -1)
                    wstd = w.std(dim=1).reshape(sz[0], 1, 1, 1)
                    wmean = w.mean(dim=1).reshape(sz[0], 1, 1, 1)

                    if args.local_rank == 0 and print_flag:
                        wm = wstd.reshape(-1).mean().item()
                        wmm = wmean.reshape(-1).mean().item()
                        print(
                            "lam = %.6f" % args.lam,
                            "mineps = %.6f" % args.mineps,
                    ...
                    d_p.add_(
                        args.lam, (1 - args.mineps / wstd) * (p.data - wmean) + wmean
                    )
...
parser.add_argument("-info", "--info", default="normal", type=str)
...
parser.add_argument(
    ...
    dest="el2",
    action="store_true",
    help="whether to use e-shifted L2 regularizer",
)
parser.add_argument(
    "--mineps",
    dest="mineps",
...
parser.add_argument(
    ...
    default=1e-4,
    type=float,
    help="lam of weights for e-shifted L2 regularizer",
)
...
parser.add_argument(
    "--mixup", dest="mixup", action="store_true", help="whether to use mixup"
)
parser.add_argument(
    "--alpha",
    default=0.2,
...
parser.add_argument(
    "--cos", dest="cos", action="store_true", help="using cosine decay lr schedule"
)
parser.add_argument(
    "--warmup", "--wp", default=5, type=int, help="number of epochs to warmup"
)
parser.add_argument(
...
parser.add_argument(
    "--wd-all", dest="wdall", action="store_true", help="weight decay on all parameters"
)
...
parser.add_argument(
    "--cardinality", type=int, default=32, help="ResNet cardinality (group)."
)
parser.add_argument("--base-width", type=int, default=4, help="ResNet base width.")
parser.add_argument(
    "--widen-factor", type=int, default=4, help="Widen factor. 4 -> 64, 8 -> 128, ..."
)
# Miscs
...
def main():
    ...
    if args.pretrained:
        ...
        model = models.__dict__[args.arch](info=args.info)
    print("=> creating model '{}'".format(args.arch))

    args.info = args.info + time.strftime("-%Y-%m-%d-%H:%M:%S", time.gmtime())
    args.checkpoint = args.checkpoint + "/" + args.arch
    args.checkpoint = args.checkpoint + "/" + args.info
    if not os.path.isdir(args.checkpoint) and args.local_rank == 0:
        mkdir_p(args.checkpoint)
    print("Model: %s\nDataset: %s\nInfo: %s\n" % (args.arch, args.data, args.info))

    flops, params = get_model_complexity_info(
        model, (224, 224), as_strings=False, print_per_layer_stat=False
    )
    ...
    args.lr = float(0.1 * float(args.train_batch * args.world_size) / 256.0)
    state["lr"] = args.lr
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_acc, train_acc5 = train(
            train_loader, model, criterion, optimizer, epoch, use_cuda
        )
        test_loss, test_acc, test_acc5 = test(
    ...
    while inputs is not None:
        ...
        if args.cutmix:
            ...
            inputs, targets_a, targets_b, lam = cutmix_data(
                inputs, targets, args.cutmix_prob, use_cuda
            )
...
def save_checkpoint(
    ...
    # filepath = os.path.join(checkpoint, str(state["epoch"]) + "." + filename)
